...
if nargin > 0
    for index = 1:2:length(varargin)
        ...
        switch field
            ...
            case {'FontType', 'Fonttype', 'fonttype'}
                fonttype = val;
...
if window.Debug == 1
    ...
    % else
    [window.Number , window.Rect] = Screen('OpenWindow', windowin, window.BGColor,debugWinRect,[],[],window.StereoMode,window.MultiSample, window.ImagingPipeline);
    %end
else
    if window.StereoMode == 4
        PsychImaging('PrepareConfiguration');
...
window.ifi          = Screen('GetFlipInterval', window.Number);
window.IsWin        = IsWin;
...
if  ~isempty(window.Blending)
    if window.Blending
        ...
        Screen('BlendFunction', window.Number, GL_ONE, GL_ZERO);
    end

end

%% Set font parameters
Screen('TextFont',  window.Number , fonttype);
