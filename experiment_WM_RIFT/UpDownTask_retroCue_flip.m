...
try % open screen from here
    ...
    [v, d, f, t] = ndgrid(views, directions, figures, targetIds);

    % Combine into a cell array
    combinations = [v(:), d(:), f(:), num2cell(t(:))];

    % Create the table
    resultTable = cell2table(combinations, ...
                    'VariableNames', {'Views', 'Directions', 'Figures', 'TargetIds'});
                
    minNTrial = length(tragetIds) * length(views)*length(directions)*length(figures)*length(angles);
    ...
    TRS.image2 = strcat('Stimuli/', TRS.direction, '-Upper-shoulder-middle/', TRS.figure2, '_', TRS.direction, '_', num2str(TRS.angle2), TRS.view2, '.png');
    ...
    img.offAng= 3;
    img.WAng  = 8;
    img.offPix = visAng2pix(img.offAng, 0, monitor);
    img.Scale = visAng2pix(img.WAng, 0, monitor)/img.W;
    img.rect = [0, 0, img.W * img.Scale, img.H * img.Scale];
    pos.L = CenterRectOnPoint( img.rect,  w.Center(1) +img.offPix , w.Center(2)); % for left-facing images: put on the right
    pos.R = CenterRectOnPoint( img.rect,  w.Center(1) -img.offPix , w.Center(2)); % for right-facing images
    for i  = 1:height(T)
    ...
    for thetrial = 1: NTrial
        ...
        for nf = 1: SwitchFr(end)
            ...
            elseif  nf <= SwitchFr(2) % target 1
                Screen( 'DrawTexture', w.Number, T.img1Txt(thetrial), [], T.pos{i}(1,:));
                Screen('DrawDots', w.Number, [0; 0], fix.Size, fix.Col, w.Center, 1);
            elseif  nf <= SwitchFr(3) % blank 1
                Screen('DrawDots', w.Number, [0; 0], fix.Size, fix.Col, w.Center, 1);
            elseif  nf <= SwitchFr(4) % target 2
                Screen( 'DrawTexture', w.Number, T.img2Txt(thetrial), [], T.pos{i}(2,:));
                Screen('DrawDots', w.Number, [0; 0], fix.Size, fix.Col, w.Center, 1);
            ...
            elseif nf <= SwitchFr(8)
                Screen('DrawTexture', w.Number, T.probeTxt(thetrial), [], T.pos{i}(3,:));                
                Screen('DrawDots', w.Number, [0; 0], fix.Size, fix.Col, w.Center, 1);
            elseif ~ButPres
