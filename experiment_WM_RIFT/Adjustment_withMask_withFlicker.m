...
if ~exist( 'prm') % this will run if the "function" line is commented
...
try % open screen from here
    ...
    prm.exp.inst{1} = ['In each trial, you will see two persons lifting their arm, one after another. \n\n',...
    ...
    prm.exp.inst{2} = ['In each trial, you will see two bars, one after another, one on the left, one on the right \n\n',...
    ...
    if prm.exp.body1st==1
        prm.exp.inst = fliplr(prm.exp.inst);
    end
     instruction_screen(prm, prm.exp.inst{1});
    ...
    for thetrial = 1: prm.N.trial
        ...
        if thetrial< prm.N.trial && mod(thetrial, prm.N.trialPerBlock)==0
            ...
            instruction_screen(prm, blockPromt);      
            ...
            blockPrompt= sprintf('When you are ready, press SPACE to start!', T.block(thetrial)-1);
            instruction_screen(prm, blockPromt);
            % make sure vbl is available for the next trial
            vbl = screen('Flip', prm.w.Number);
            
           
            % if the middle of the task: also instruct the new stimuli
            if thetrial == prm.N.trial/2 % start the new stimulus
                instruction_screen(prm, prm.exp.inst{2}); 
                % make sure vbl is available for the next trial
                vbl = screen('Flip', prm.w.Number);
            end
        end
