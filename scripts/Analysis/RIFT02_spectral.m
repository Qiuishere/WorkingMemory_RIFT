function RIFT02_spectral(subj_id)
    ...
    % cfg =[];
    % cfg.demean = 'yes';
    % cfg.baselinewindow = [-0.3 -0.2];
    % cfg.channel = {'tag', 'UADC001'};
    % dattag = ft_preprocessing(cfg, data_MEG_corrected);

    % %%
    % cfg = [];
    % cfg.viewmode= 'butterfly';
    % ft_databrowser(cfg, dattag);

    
    %% 0) check the photodiode
    cfg = [];
    cfg.latency = [0.2,prm.dur.delay/1000-0.2-1/data_diode.fsample];
    delay_diode = ft_selectdata(cfg, data_diode);
    ...
    cfg.method  = 'mtmfft';
    cfg.taper   = 'hanning';
    cfg.foi     = 1:1:90; % 1/cfg1.length  = 1;
    cfg.keeptrials = 'no';

    
    PD_freq  = ft_freqanalysis(cfg, delay_diode);



    % plot the results
    figure
    fg = plot(PD_freq.freq, PD_freq.powspctrm());
    title('PSD for photo diode')
    saveas(fg,fullfile(subjects(subj_id).results, '/02_spectral//photo_diode.jpg'));

    % check if the peak is indeed the same as tagged frequency
    peak_diode  = PD_freq.freq(PD_freq.powspctrm==max(PD_freq.powspctrm));
    sprintf('Peak of photodiode is %g hz', peak_diode)

    assert(peak_diode == PDtagged_freq,'photodiode does not match tagging');


    %% get the real onset of the photodiode

    for i = 1:Ntrials
        thedata = data_diode.trial{i};
        baseline = mean(thedata(t_points<0 & t_points> -0.5));
        searchRange = t_points>=0 & t_points<0.05;
        id1 = find(abs(thedata - baseline) > 0.2 & searchRange==1, 1, 'first') -1 ; % -1 because the signal already started
        onset_t(i) = t_points(id1);

        % look for valley
        searchRange = t_points>=onset_t(i) & t_points<onset_t(i) + 1/PDtagged_freq;
        valley = min(thedata(searchRange));
        peakpoints = find(thedata ==valley);
        valley_t(i) = t_points(peakpoints);

    end

    figure
    scatter(ones(Ntrials,1)+0.2*rand(Ntrials,1), onset_t)
        figure
    scatter(1:Ntrials, valley_t)


    phase_tag = 2*pi*PDtagged_freq* (onset_t - valley_t);
    phase_tag = mod(phase_tag, 2*pi);
    figure
    scatter(ones(Ntrials,1)+0.2*rand(Ntrials,1), phase_tag)
    

    for i = 11:10:60
        figure
        plot(data_diode.time{i}(700:900), data_diode.trial{i}(700:900))
    ...
    for i = 20:10:200
        ...
        t = find(data_diode.time{1}==onset(i));

        plot(1:101, data_diode.trial{i}(t:t+100));
        hold on
    ...
    % data_cmb = ft_appenddata([], data_MEG, data_tagsig);
    
    
    %% simple check with databrowser
    
    cfg = [];
    cfg.viewmode= 'butterfly';
    cfg.channel = {'UADC001', ['tag_' num2str(PDtagged_freq)]};
    ft_databrowser(cfg, data_cmb);
