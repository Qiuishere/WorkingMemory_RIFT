...
% Collection info
f_s = 250; % Hz
trial_latency = [-1.5, +2.5]; % sec, has to include 0
...
for p = 1:length(participants)
    for eye = 1:2
        x_deviation(:, :, eye, p) = x_pos_unmatched(:, [floor(1.5*f_s):floor(3*f_s)], eye, p) - mean(x_pos_unmatched(:, :, eye, p), "all", "omitnan");
        y_deviation(:, :, eye, p) = y_pos_unmatched(:, [floor(1.5*f_s):floor(3*f_s)], eye, p) - mean(y_pos_unmatched(:, :, eye, p), "all", "omitnan");        
...
% Portion to select
ccl = [-1.5, 1.3]; % cue-centered latency, sec, must include 0
...
for p = 1:length(participants)
    ...
    post_trigger_time_sec = -trial_latency(1) + 1.15;
    ...
    for tr = 1:num_trials
        x_pos_uncorrected(tr, :, :, p) = x_pos_unmatched(tr, post_trigger_time_eytckr + (ccl(1)*f_s):post_trigger_time_eytckr+(ccl(2)*f_s), :, p);
        y_pos_uncorrected(tr, :, :, p) = y_pos_unmatched(tr, post_trigger_time_eytckr + (ccl(1)*f_s):post_trigger_time_eytckr+(ccl(2)*f_s), :, p);
    end
end
...
nan_free_period = [0, 1] + (-ccl(1)); % cue centered;
...
xlim([-0.5 1.3]); ylabel("Deviation (dva)"); xticks([]); xticklabels([]); legend("Left Cued", "Right Cued");
...
xlim([-0.5 1.3]); xlabel("Time (s) w.r.t. stimuli onset"); ylabel("Difference (dva)");
y_val = 0.1; ylim([-y_val y_val]); yticks([-0.05, 0, 0.05]);
xline(0, 'k--', 'HandleVisibility', 'off'); yline(0, 'k-', 'HandleVisibility', 'off', 'LineWidth', 2);

sgtitle("Pre-cue Task eye movements, x-direction")

if 0 
    save FigsDataPython/Eye_Pre/t.mat t
    save FigsDataPython/Eye_Pre/ci.mat ci
    save FigsDataPython/Eye_Pre/x_pos_dva_p_avgd.mat x_pos_dva_p_avgd
    save FigsDataPython/Eye_Pre/sig_traces.mat sig_traces
    save FigsDataPython/Eye_Pre/ci_diff.mat ci_diff
    save FigsDataPython/Eye_Pre/x_pos_diff_trace.mat x_pos_diff_trace
    save FigsDataPython/Eye_Pre/sig_clusters.mat sig_clusters
end
...
xlim([-0.5 1.3]); ylabel("Deviation (dva)"); xticks([]); xticklabels([]);
ylim([-y_val y_val+0.001]); xline(0, 'k--', 'HandleVisibility', 'off');  yline(0, 'k-', 'HandleVisibility', 'off', 'LineWidth', 2);
...
sgtitle("Pre-cue Task eye movements, y-direction")
